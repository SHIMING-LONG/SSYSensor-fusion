...
%
  ...
  % ----------------------- OUR FILTER SETTINGS ------------------------
  t_old = 0;
  Rw = diag([cov(calGyr(:,1)), cov(calGyr(:,2)), cov(calGyr(:,3)) ])*1;
  Ra = diag([cov(calAcc(:,1)), cov(calAcc(:,2)), cov(calAcc(:,3)) ])*1;
  Rm = diag([cov(calMag(:,1)), cov(calMag(:,2)), cov(calMag(:,3)) ])*1;
  ...
  meas = struct('t', zeros(1, 0),...
    ...
    while m.Connected && m.Logging 
      ...
      if m.AngularVelocitySensorEnabled
        ...
        % Get time interval
        T = t-t_old;
        gyr_corrected = [gyr(1); gyr(2); gyr(3)];

        % Update the state estimate (task5)
        [x, P] = tu_qw(x, P, gyr_corrected, T, Rw);
     ...
     if  m.MagneticSensorEnabled
      ...
      orientation = eul2quat([-(dataOrient(1) - azimuth0), dataOrient(3), -dataOrient(2)].*(pi/180),"ZYX");
      % Visualize result
      if rem(counter, 10) == 0
